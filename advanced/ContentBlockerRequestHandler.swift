...
class ContentBlockerRequestHandler: NSObject, NSExtensionRequestHandling {
    ...
    func beginRequest(with context: NSExtensionContext) {
        let rulesURL = getAdvancedRulesURL()
    ...
    /// Получает URL файла правил advanced с fallback к bundle
    private func getAdvancedRulesURL() -> URL? {
        ...
        if let groupURL = fileManager.containerURL(forSecurityApplicationGroupIdentifier: appGroupID) {
            let appGroupURL = groupURL.appendingPathComponent("advanced.json")
